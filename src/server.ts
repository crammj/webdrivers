...
cron.schedule(daily, () => {
    ...
    exec('npx jest tests/jobs/manage-club-reg.test.ts', (error, stdout, stderr) => {
        if (error) {
...
cron.schedule(daily, () => {
    console.log('Running specific test file at midnight');
    exec('npx jest tests/jobs/manage-tourney-reg.test.ts', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error running test: ${error.message}`);
            return;
        }
...
cron.schedule(fridayEvening, () => {
    console.log('Running specific test file at midnight');
    exec('npx jest tests/jobs/start-tourney-email.test.ts', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error running test: ${error.message}`);
            return;
        }
...
cron.schedule(tuesdayMorning, () => {
    console.log('Running specific test file at midnight');
    exec('npx jest tests/jobs/schedule-next-tourney.test.ts', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error running test: ${error.message}`);
            return;
        }
...
cron.schedule(tuesdayMorning2, () => {
    console.log('Running specific test file at midnight');
    exec('npx jest tests/jobs/post-tourney-article.test.ts', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error running test: ${error.message}`);
            return;
        }
